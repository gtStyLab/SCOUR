function [regScheme_2contMet,regScheme_3contMet,regScheme_4contMet] = createRegSchemeList(S,fluxes_to_remove)
% Create list of possible regulatory interactions that are tested by SCOUR.
...
for flux = fluxes
    ...
    for i=1:length(scheme)
        ...
        if exist('MA_2contMet')
            ...
            all_controller_mets = sort(unique([MA_1contMet regScheme_tarFlux(scheme(i,1),1) regScheme_tarFlux(scheme(i,2),1)]));
        end
...
for flux = fluxes
    ...
    for i=1:length(scheme)
        ...
        if exist('MA_2contMet')
            ...
            all_controller_mets = sort(unique([MA_1contMet regScheme_tarFlux(scheme(i,1),1) regScheme_tarFlux(scheme(i,2),1) regScheme_tarFlux(scheme(i,3),1)]));
        end
