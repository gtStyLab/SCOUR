function fluxSamePercent = feature_functionalityPercentage_3contMet(prefix,met1,met2,met3,tarFlux,num_IC,rep)
...
for i = 1:size(allIC_concMatrix,1)
    ...
    same_x1 = find(allIC_concMatrix(:,1) > 0.95*current_x1 & allIC_concMatrix(:,1) < 1.05*current_x1);
    same_x2 = find(allIC_concMatrix(same_x1,2) > 0.95*current_x2 & allIC_concMatrix(same_x1,2) < 1.05*current_x2);
    same_x3 = find(allIC_concMatrix(same_x1(same_x2),3) > 0.95*current_x3 & allIC_concMatrix(same_x1(same_x2),3) < 1.05*current_x3);
