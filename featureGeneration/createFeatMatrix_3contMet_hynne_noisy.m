...
for regListIndex = 1:length(Hynne_regScheme_3contMet)
    ...
    % Surface fit RMSE
    [~, featureMatrix_3contMet_Hynne(1,regListIndex)] = feature_surfaceFit_multiContMet_noisy(Hynne_prefix,remaining_mets,met1,tarFlux,num_IC,nT,cov,rep);

   % Percentage method functionality
    featureMatrix_3contMet_Hynne(2,regListIndex) = feature_functionalityPercentage_3contMet_noisy(Hynne_prefix,met1,remaining_mets(1),remaining_mets(2),tarFlux,num_IC,nT,cov,rep);
    
    % Predict flux from metabolites
    featureMatrix_3contMet_Hynne(3,regListIndex) = feature_predictFlux_multiContMet_noisy(Hynne_prefix,met1,remaining_mets,tarFlux,num_IC,nT,cov,rep);
    
    % Rounding method functionality
    featureMatrix_3contMet_Hynne(4,regListIndex) = feature_functionalityRounding_3contMet_noisy(Hynne_prefix,[met1,remaining_mets],tarFlux,num_IC,nT,cov,rep);
    
    % Hold 2 metabolites constant and calculate correlation
    [featureMatrix_3contMet_Hynne(5,regListIndex)] = feature_hold2MetConstant_correlation_3contMet_noisy(Hynne_prefix,met1,remaining_mets(1),remaining_mets(2),tarFlux,num_IC,nT,cov,rep);
