...
for regListIndex = 1:length(Chass_regScheme_2contMet)
    ...
    % Functionality feature
    [featureMatrix_2contMet_Chass(1,regListIndex)] = feature_functionality_2contMet_noisy(Chass_prefix,met1,met2,tarFlux,num_IC,nT,cov,rep);
    
    % Surface fit RMSE
    [~, featureMatrix_2contMet_Chass(2,regListIndex)] = feature_surfaceFit_multiContMet_noisy(Chass_prefix,met1,met2,tarFlux,num_IC,nT,cov,rep);

    % Predict flux with machine learning
    featureMatrix_2contMet_Chass(3,regListIndex) = feature_predictFlux_multiContMet_noisy(Chass_prefix,met1,met2,tarFlux,num_IC,nT,cov,rep);
    
    % Hold 1 metabolite constant and calculate correlation
    [featureMatrix_2contMet_Chass(4,regListIndex)] = feature_hold1MetConstant_correlation_2contMet_noisy(Chass_prefix,met1,met2,tarFlux,num_IC,nT,cov,rep);
    
    % Hold 1 metabolite constant and fit curve
    [~, featureMatrix_2contMet_Chass(5,regListIndex)] = feature_hold1MetConstant_curveFit_2contMet_noisy(Chass_prefix,met1,met2,tarFlux,num_IC,nT,cov,rep);
end
