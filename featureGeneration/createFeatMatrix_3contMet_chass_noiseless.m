...
for regListIndex = 1:length(Chass_regScheme_3contMet)
    ...
    % Surface fit RMSE
    [~, featureMatrix_3contMet_Chass(1,regListIndex)] = feature_surfaceFit_multiContMet(Chass_prefix,remaining_mets,met1,tarFlux,num_IC,rep);

   % Percentage method functionality
    featureMatrix_3contMet_Chass(2,regListIndex) = feature_functionalityPercentage_3contMet(Chass_prefix,met1,remaining_mets(1),remaining_mets(2),tarFlux,num_IC,rep);
    
    % Predict flux from metabolites
    featureMatrix_3contMet_Chass(3,regListIndex) = feature_predictFlux_multiContMet(Chass_prefix,met1,remaining_mets,tarFlux,num_IC,rep);
    
    % Rounding method functionality
    featureMatrix_3contMet_Chass(4,regListIndex) = feature_functionalityRounding_3contMet(Chass_prefix,[met1,remaining_mets],tarFlux,num_IC,rep);
    
    % Hold 2 metabolites constant and calculate correlation
    [featureMatrix_3contMet_Chass(5,regListIndex)] = feature_hold2MetConstant_correlation_3contMet(Chass_prefix,met1,remaining_mets(1),remaining_mets(2),tarFlux,num_IC,rep);
