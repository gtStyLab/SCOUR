function dataPreparation_Synthetic_noisy(nT,cov,num_IC,reps)
% Generate triplicate noisy data for synthetic models. Then apply a
...
% Generate noisy BiggerModel data
fprintf('Generating noisy BiggerModel testing data...\n');
for IC = 1:num_IC
    wrapper_genNoisyData_3samples(sprintf('BiggerModelData/odeData/BiggerModel_k-%02d_hiRes.mat',IC),nT,cov,reps)
end

% Generate noisy SmallerModel data
fprintf('Generating noisy SmallerModel testing data...\n');
for IC = 1:num_IC
    wrapper_genNoisyData_3samples(sprintf('SmallerModelData/odeData/SmallerModel_k-%02d_hiRes.mat',IC),nT,cov,reps)
end

%% Smooth BiggerModel data
fprintf('Smoothing BiggerModel testing data...\n')
...
for rep = 1:reps
    for IC = 1:num_IC
        ...
        for s = 1:3
            filename = sprintf('BiggerModelData/odeData/%s_k-%02d_nT-%03d_cov-%02d_s%01d_rep-0%02d.mat',prefix,IC,nT,cov,s,rep);
        ...
        fileOutName = sprintf('BiggerModelData/odeData/%s_k-%02d_nT-%03d_cov-%02d_rep-0%02d_smooth.mat',prefix,IC,nT,cov,rep);
        save(fileOutName,'-struct','odeData');
    end
end

% Smooth SmallerModel data
fprintf('Smoothing SmallerModel testing data...\n')
smoothing_method = 'gaussian';
prefix = 'SmallerModel';
for rep = 1:reps
    for IC = 1:num_IC
        ...
        v0_fluxMatrix_3samples = [];
        for s = 1:3
            filename = sprintf('SmallerModelData/odeData/%s_k-%02d_nT-%03d_cov-%02d_s%01d_rep-0%02d.mat',prefix,IC,nT,cov,s,rep);
            odeData = load(filename);
            concMatrix_3samples = [concMatrix_3samples; odeData.concMatrix(2:end,:)];
            timeVec_3samples = [timeVec_3samples; odeData.timeVec(2:end)];
            fluxMatrix_3samples = [fluxMatrix_3samples; odeData.fluxMatrix(2:end,:)];
        ...
        fileOutName = sprintf('SmallerModelData/odeData/%s_k-%02d_nT-%03d_cov-%02d_rep-0%02d_smooth.mat',prefix,IC,nT,cov,rep);
        save(fileOutName,'-struct','odeData');
    end
end

%% Median BiggerModel data
fprintf('Calculating BiggerModel testing data medians...\n')
prefix = 'BiggerModel';
for rep = 1:reps
    for IC = 1:num_IC
        concMatrix_3samples = [];
        ...
        fileOutName = sprintf('BiggerModelData/odeData/%s_k-%02d_nT-%03d_cov-%02d_rep-0%02d_median.mat',prefix,IC,nT,cov,rep);
        save(fileOutName,'-struct','odeData');
    end
end

% Median SmallerModel data
fprintf('Calculating SmallerModel testing data medians...\n')
prefix = 'SmallerModel';
for rep = 1:reps
    for IC = 1:num_IC
        concMatrix_3samples = [];
        ...
        fileOutName = sprintf('SmallerModelData/odeData/%s_k-%02d_nT-%03d_cov-%02d_rep-0%02d_median.mat',prefix,IC,nT,cov,rep);
        save(fileOutName,'-struct','odeData');
    end
end
