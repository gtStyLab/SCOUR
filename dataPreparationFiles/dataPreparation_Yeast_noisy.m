function dataPreparation_Yeast_noisy(nT,cov,num_IC,reps)
% Generate triplicate noisy data for yeast model. Then apply a
...
% Generate noisy yeast data
fprintf('Generating noisy yeast testing data...\n');
for IC = 1:num_IC
    wrapper_genNoisyData_3samples(sprintf('HynneData/odeData/hynne_k-%02d_hiRes.mat',IC),nT,cov,reps)
end
...
for rep = 1:reps
    for IC = 1:num_IC
        ...
        for s = 1:3
            filename = sprintf('HynneData/odeData/%s_k-%02d_nT-%03d_cov-%02d_s%01d_rep-0%02d.mat',prefix,IC,nT,cov,s,rep);
        ...
        fileOutName = sprintf('HynneData/odeData/%s_k-%02d_nT-%03d_cov-%02d_rep-0%02d_smooth.mat',prefix,IC,nT,cov,rep);
        save(fileOutName,'-struct','odeData');
    end
end

%% Median yeast data
fprintf('Calculating yeast testing data medians...\n')
prefix = 'hynne';
for rep = 1:reps
    for IC = 1:num_IC
        concMatrix_3samples = [];
        ...
        fileOutName = sprintf('HynneData/odeData/%s_k-%02d_nT-%03d_cov-%02d_rep-0%02d_median.mat',prefix,IC,nT,cov,rep);
        save(fileOutName,'-struct','odeData');
    end
end
